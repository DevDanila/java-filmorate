...
class UserControllerTest extends BaseControllerTest {
	...
	void addNoValidUserTest() throws IOException, InterruptedException {
		...
		// Уберите проверку GET-запроса, так как невалидные пользователи не должны добавляться
		user = setDefaultUser();
		user.setName(null);
		userJson = objectMapper.writeValueAsString(user);
		httpResponse = getResponse(path, RequestMethod.POST, userJson);
		assertEquals(200, httpResponse.statusCode(), "Не работает подмена имени логином");
		User responseUser = objectMapper.readValue(httpResponse.body(), User.class);
		assertEquals(user.getLogin(), responseUser.getName(), "Имя не было заменено логином");
	}
}